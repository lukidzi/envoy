...
TEST(AccessLogFormatterTest, plainStringFormatter) {
  ...
  EXPECT_EQ("plain", formatter.format(header, header, header, stream_info, body));
}
...
TEST(AccessLogFormatterTest, streamInfoFormatter) {
...

  {
    ...
    EXPECT_EQ("5", request_duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_CALL(stream_info, lastDownstreamRxByteReceived()).WillOnce(Return(dur));
    EXPECT_EQ("-", request_duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("10", response_duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    absl::optional<std::chrono::nanoseconds> dur;
    EXPECT_CALL(stream_info, firstUpstreamRxByteReceived()).WillRepeatedly(Return(dur));
    EXPECT_EQ("-", response_duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("15", ttlb_duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("-", ttlb_duration_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter bytes_received_format("BYTES_RECEIVED");
    EXPECT_CALL(stream_info, bytesReceived()).WillOnce(Return(1));
    EXPECT_EQ("1", bytes_received_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("HTTP/1.1", protocol_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("200", response_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_CALL(stream_info, responseCode()).WillRepeatedly(Return(response_code));
    EXPECT_EQ("0", response_code_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("-", response_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_CALL(stream_info, responseCodeDetails()).WillRepeatedly(ReturnRef(rc_details));
    EXPECT_EQ("via_upstream", response_code_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter bytes_sent_format("BYTES_SENT");
    EXPECT_CALL(stream_info, bytesSent()).WillOnce(Return(1));
    EXPECT_EQ("1", bytes_sent_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("15", duration_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("LR", response_flags_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter upstream_format("UPSTREAM_HOST");
    EXPECT_EQ("10.0.0.1:443", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ("cluster_name", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter upstream_format("UPSTREAM_HOST");
    EXPECT_CALL(stream_info, upstreamHost()).WillOnce(Return(nullptr));
    EXPECT_EQ("-", upstream_format.format(header, header, header, stream_info, body));
  }
...

  {
    ...
    EXPECT_EQ("127.0.0.2:0", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_LOCAL_ADDRESS_WITHOUT_PORT");
    EXPECT_EQ("127.0.0.2", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_REMOTE_ADDRESS_WITHOUT_PORT");
    EXPECT_EQ("127.0.0.1", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_REMOTE_ADDRESS");
    EXPECT_EQ("127.0.0.1:0", upstream_format.format(header, header, header, stream_info, body));
  }
...

  {
    ...
    EXPECT_EQ("stub_server", upstream_format.format(header, header, header, stream_info, body));
  }
  ...
  }
  {
    ...
    EXPECT_EQ("san", upstream_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_CALL(*connection_info, uriSanPeerCertificate()).WillRepeatedly(Return(sans));
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ("san1,san2", upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_PEER_URI_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ("subject", upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_LOCAL_SUBJECT");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ("deadbeef", upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_TLS_SESSION_ID");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_EQ("TLS_DHE_RSA_WITH_AES_256_GCM_SHA384",
              upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_TLS_CIPHER");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ("TLSv1.2", upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_TLS_VERSION");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ(expected_sha, upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_PEER_FINGERPRINT_256");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ("b8b5ecc898f2124a", upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_PEER_SERIAL");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_CALL(stream_info, downstreamSslConnection()).WillRepeatedly(Return(connection_info));
    EXPECT_EQ(expected_cert, upstream_format.format(header, header, header, stream_info, body));
  }
  {
    StreamInfoFormatter upstream_format("DOWNSTREAM_PEER_CERT");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  }
  {
    ...
    EXPECT_EQ("SSL error", upstream_format.format(header, header, header, stream_info, body));
  }
...
TEST(AccessLogFormatterTest, startTimeFormatter) {
...

  {
    ...
    EXPECT_EQ("2018/03/28", start_time_format.format(header, header, header, stream_info, body));
  }

  {
    ...
    EXPECT_EQ(AccessLogDateTimeFormatter::fromTime(time),
              start_time_format.format(header, header, header, stream_info, body));
  }
}

TEST(AccessLogFormatterTest, bodyFormatterTest) {
  NiceMock<StreamInfo::MockStreamInfo> stream_info;
  Http::TestHeaderMapImpl header{{":method", "GET"}, {":path", "/"}};
  std::string body = "My test message";

  {
    BodyFormatter body_formatter{};
    EXPECT_EQ("My test message", body_formatter.format(header, header, header, stream_info, body));
  }
}
...
TEST(AccessLogFormatterTest, JsonFormatterPlainStringTest) {
  ...
  verifyJsonOutput(formatter.format(request_header, response_header, response_trailer, stream_info, body),
                   expected_json_map);
}
...
TEST(AccessLogFormatterTest, CompositeFormatterSuccess) {
  ...
  {
    const std::string format = "%RESP_BODY% - with response body";
    FormatterImpl formatter(format);

    EXPECT_EQ("Test body - with response body",
              formatter.format(request_header, response_header, response_trailer, stream_info, body));
  }
